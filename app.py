...
def login():
    ...
    has_baseline = db.training.find_one({"userId": {"$regex": f"^{real_username}$", "$options": "i"}}) is not None
...
def generate_user_report_pdf(username):
    ...
    return send_file(buffer, as_attachment=True, download_name=f"{username}_full_report.pdf", mimetype="application/pdf")
